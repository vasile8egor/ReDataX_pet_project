...
def date_bounds(values: list[str]) -> tuple[date, date]:
    if not values:
...
def same_candidate(left: dict[str, Any] | None, right: dict[str, Any] | None) -> bool:
...
def recommendation_matches(candidate: dict[str, Any], selected: dict[str, Any] | None) -> bool:
    if selected is None:
        return False
    return (
...
def delete_experiment(client: Any, table: str, experiment_id: str) -> None:
...
def replace_registry(client: Any) -> None:
    model_ids = ", ".join(f"'{row[0]}'" for row in MODEL_REGISTRY_ROWS)
    policy_ids = ", ".join(f"'{row[0]}'" for row in POLICY_REGISTRY_ROWS)

    client.execute(
        f"ALTER TABLE gold.dim_research_model_registry "
        f"DELETE WHERE model_id IN ({model_ids}) "
        "SETTINGS mutations_sync = 1"
    )
    client.execute(
        f"ALTER TABLE gold.dim_research_policy_registry "
        f"DELETE WHERE policy_id IN ({policy_ids}) "
        "SETTINGS mutations_sync = 1"
    )
