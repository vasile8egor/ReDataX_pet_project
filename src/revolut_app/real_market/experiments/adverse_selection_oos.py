...
from revolut_app.real_market.experiments.queries import BTC_REF_SQL, TRADES_SQL
...
def load_day(ch: Client, trade_date: date, symbol: str):
    cols = ch.execute(
        TRADES_SQL,
...
def causal_imbalances(data: DayData):
...
def feature_matrices(data: DayData, phi: np.ndarray):
    valid = (
...
def bootstrap(values: list[float], samples: int, seed: int):
...
def write_output(path: str, output: dict[str, object]):
