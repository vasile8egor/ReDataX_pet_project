#!/usr/bin/env bash

set -euo pipefail

API_URL="${API_URL:-http://localhost:8000}"

SOURCE_MODEL_VERSION="${SOURCE_MODEL_VERSION:-hamiltonian-observer-v1-heldout}"
MODEL_VERSION="${MODEL_VERSION:-hamiltonian-observer-v1-rg-diagnostic-b16-v3}"

docker compose exec -T api python - <<'PY'
from revolut_app.loaders.rg_transition_diagnostics_loader import (
    RgTransitionDiagnosticsLoader,
)

loader = RgTransitionDiagnosticsLoader()
loader.ensure_schema()

print("RG transition diagnostics schema created")
PY

mapfile -t DATASET_IDS < <(
  docker exec clickhouse clickhouse-client \
    --user default \
    --password default \
    --query "
      SELECT DISTINCT
          toString(event_dataset_id)
      FROM gold.fact_simulation_runs
      WHERE model_version =
          '${SOURCE_MODEL_VERSION}'
      ORDER BY event_dataset_id
      FORMAT TSVRaw
    "
)

if [[ "${#DATASET_IDS[@]}" -ne 10 ]]; then
  echo "Expected 10 held-out datasets."
  echo "Found ${#DATASET_IDS[@]}."
  exit 1
fi

for dataset_id in "${DATASET_IDS[@]}"; do
  echo
  echo "Diagnostic replay: ${dataset_id}"

  curl --fail-with-body \
    --silent \
    --show-error \
    --output /dev/null \
    --write-out \
      "http=%{http_code} total=%{time_total}s\n" \
    --request POST \
    "${API_URL}/fx/policy-comparison" \
    --header "Content-Type: application/json" \
    --data "{
      \"event_dataset_id\": \"${dataset_id}\",
      \"policies\": [
        \"naive\",
        \"inventory_aware\",
        \"platform\"
      ],
      \"amount_multiplier\": 150,
      \"snapshot_every_n_events\": 1,
      \"persist_result\": true,
      \"model_version\": \"${MODEL_VERSION}\",
      \"physics_mode\": \"observer\",
      \"hamiltonian_preset\": \"local-v1\",
      \"scale_aware_diagnostic_preset\": \"rg-local-b16-v1\",
      \"scale_aware_diagnostic_epsilon\": 0.000001,
      \"hedging_policy\": \"none\"
    }"
done

echo
echo "RG B16 diagnostic replay completed."
